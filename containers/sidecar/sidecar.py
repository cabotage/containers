...
def token_revoke_self(vault_ca_file, vault_addr, token):
    response = requests.post(
        f"{vault_addr}/v1/auth/token/revoke-self",
        headers={"X-Vault-Token": token},
        json={},
        verify=vault_ca_file,
    )
...
def leases_renew(vault_ca_file, vault_addr, token, lease_id):
    response = requests.put(
        f"{vault_addr}/v1/sys/leases/renew",
        headers={"X-Vault-Token": token},
        json={"lease_id": lease_id},
        verify=vault_ca_file,
    )
...
def vault_auth_kubernetes_login(
    vault_ca_file, vault_addr, vault_backend, vault_role, jwt, wrap, unwrap
):
    headers = {}
    if wrap:
        headers["X-Vault-Wrap-TTL"] = "60s"
    token = requests.post(
        f"{vault_addr}/v1/{vault_backend}",
        headers=headers,
...
def disposable_vault_token(vault_ca_file, vault_addr, vault_backend, vault_role, jwt):
    ...
    try:
        ...
        token = vault_auth_kubernetes_login(
            vault_ca_file, vault_addr, vault_backend, vault_role, jwt, True, True
        )
...
def request_vault_certificate(
    ...
    response = requests.post(
        f"{vault_addr}/v1/{vault_pki_backend}/issue/{vault_pki_role}",
        json={
...
def write_key_material(cert_dir, cert_object):
    ...
    with open(os.path.join(cert_dir, "cert.pem"), "wb") as cert_file:
        cert_file.write(certificate.encode("utf-8") + b"\n")
    with open(os.path.join(cert_dir, "combined.pem"), "wb") as combined_file:
        combined_file.write(certificate.encode("utf-8") + b"\n")
        combined_file.write(issuing_ca.encode("utf-8") + b"\n")
        combined_file.write(private_key.encode("utf-8") + b"\n")
    with open(os.path.join(cert_dir, "ca.pem"), "wb") as ca_file:
        ca_file.write(issuing_ca.encode("utf-8") + b"\n")
...
def read_cert(cert_dir):
    ...
    ip_sans = [
        str(ip) for ip in subject_alternative_names.get_values_for_type(x509.IPAddress)
    ]

    return common_name, dns_names, ip_sans


def vault_fetch_certificate(
    vault_addr,
    vault_token,
    vault_ca_file,
    vault_pki_backend,
    ...
    certificate_response = request_vault_certificate(
        vault_addr,
        vault_token,
        vault_ca_file,
        vault_pki_backend,
...
def write_consul_token(consul_secrets_path, consul_token_object):
    ...
    lease_sha = hashlib.sha256(lease_id.encode("utf-8")).hexdigest()
    with open(
        os.path.join(consul_secrets_path, "leases", lease_sha), "wb"
    ) as lease_file:
...
def vault_fetch_consul_token(
    ...
    consul_token_response = request_consul_token(
        vault_addr,
        token_contents,
        vault_ca_file,
        vault_consul_backend,
...
@click.option(
    "--vault-addr",
    default="https://vault.cabotage.svc.cluster.local",
    help="Vault address to communicate with.",
)
@click.option(
    "--vault-ca-file",
    default="/var/run/secrets/cabotage.io/ca.crt",
...
@click.option(
    "--vault-token-file",
    default="/var/run/secrets/vault/vault-token",
    help="Path Vault Token is stored at",
...
@click.option(
    "--cert-dir",
    default="/var/run/secrets/vault",
    help="Path Vault Issued Certificate and Key are stored at",
    type=click.Path(),
)
@click.option(
    "--vault-pki-backend",
...
@click.option(
    "--unwrap/--no-unwrap",
    default=False,
    help="Unwrap stored vault token, may not be desirable for some apps",
)
@click.option(
    "--write-vault-token-file",
    default="/var/run/secrets/vault/vault-token",
    help="File to write unrwapped Vault Token to",
...
def maintain(
    ...
    if unwrap:
        ...
        try:
            ...
            click.Abort()
        vault_token = unwrap_vault_response(
