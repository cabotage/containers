...
@click.option(
    "--pod-ip", required=True, help="pod IP address as defined by pod.status.podIP"
)
@click.option(
    "--additional-dnsnames", default="", help="additional dns names; comma separated"
)
@click.option(
    "--service-names",
    default="",
...
def main(
    ...
    certificate_signing_request = kubernetes.client.models.V1CertificateSigningRequest(
        metadata=metadata,
        spec=certificate_signing_request_spec,
    )

    try:
        response = certificates_api.create_certificate_signing_request(
    ...
    while True:
        ...
        try:
            response = certificates_api.read_certificate_signing_request(
                csr_name, pretty=True
            )
        except ApiException as e:
            click.echo("Error from Kubernetes API: %s" % (e,))
